...
def get_shap_rewards(model, query_str, response_str, tokenizer, masker=None):
    def f(x):
        ...
        # print("REWARD LOGITS SHAPE", reward_logits.shape)
        # sequence_lengths = first_true_indices(inputs["input_ids"] == tokenizer.pad_token_id) - 1
        # output = reward_logits[torch.arange(inputs["input_ids"].shape[0], device=inputs["input_ids"].device), sequence_lengths].squeeze(-1)
        output = reward_logits.squeeze(-1)
...
def parse_sentence(paragraph, return_offsets_mapping=True):
    ...
    Returns:
        dict with:
            input_ids (list[str])
            offset_mapping (list[tuple])  # optional
    """
